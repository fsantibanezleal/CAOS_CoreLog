...
export default function Implementation() {
  ...
  return (
    <article className="page-body prose">
      ...
      <p className="lede">{es
        ? 'CoreLog está instanciado sobre el arquetipo de repo-producto CAOS (ADR-0057): dos contratos de datos, un pipeline por etapas, el gate de lane, y un frontend SPA que ejecuta la CV en vivo.'
          ...
          content: (
            <div className="pf-doc-sec">
              <ul className="pf-list">
                ...
                <li><b>{es ? 'Offline (precompute)' : 'Offline (precompute)'}</b>, {es ? 'un precálculo Node ejecuta el mismo engine TS sobre los casos · data/derived/case-results.json; torch entrena el CNN + el AE OOD · ONNX.' : 'a Node bake runs the same TS engine over the cases · data/derived/case-results.json; torch trains the CNN + the OOD AE · ONNX.'}</li>
          ...
          content: (
            <div className="pf-doc-sec">
              ...
              <ul className="pf-list">
                ...
                <li><b>real-litho-cnn.onnx</b> (~3.7 MB): {es ? 'MobileNetV3-Small congelado (ImageNet) + cabeza lineal, entrada 224 px, salida softmax DCID-7. Una inferencia por parche real (sin bomba de cómputo).' : 'frozen MobileNetV3-Small (ImageNet) + a linear head, 224 px input, DCID-7 softmax output. One inference per real patch (no compute bomb).'}</li>
                <li><b>ood-bench.json</b> (~14 KB): {es ? 'la tabla completa de detectores, curvas ROC, histogramas, controles negativos y la confusión de la cabeza, que lee la página Benchmark.' : 'the full detector table, ROC curves, histograms, negative controls and the head confusion, read by the Benchmark page.'}</li>
              </ul>
              <Callout variant="note" title={es ? 'Por qué se precomputan los embeddings' : 'Why the embeddings are precomputed'}>
                {es
                  ? 'DINOv2/MAE son el techo SOTA pero demasiado pesados para el navegador; se documentan, no se embarcan. Solo se embarcan estadísticos OOD compactos + la cabeza pequeña. El backbone MobileNet (~3.7 MB) sí cabe y se carga bajo demanda solo en el carril Real.'
