      ...
      <Tabs ariaLabel={es ? 'implementación' : 'implementation'} tabs={[
          ...
          content: (
            <div className="pf-doc-sec">
              <ul className="pf-list">
                ...
                <li><b>{es ? 'Offline (precompute)' : 'Offline (precompute)'}</b>, {es ? 'un precálculo Node corre el MISMO engine TS sobre los casos · data/derived/case-results.json; torch entrena el CNN + el AE OOD · ONNX.' : 'a Node bake runs the SAME TS engine over the cases · data/derived/case-results.json; torch trains the CNN + the OOD AE · ONNX.'}</li>
        ...
        },
        {
          ...
          content: (
            <div className="pf-doc-sec">
              <p>{es
                ? 'El detector OOD en espacio de features y la cabeza real DCID-7 se precalculan offline (data-pipeline/cllab/science/ood_bench.py, .venv-precompute + torch) y embarcan como artefactos compactos. El contrato es: backbone congelado · embedding · {estadisticos de Mahalanobis, banco kNN, cabeza lineal}.'
              ...
              <ul className="pf-list">
                ...
                <li><b>ood-bench.json</b> (~14 KB): {es ? 'la tabla completa de detectores, curvas ROC, histogramas, controles negativos y la confusion de la cabeza, que lee la página Benchmark.' : 'the full detector table, ROC curves, histograms, negative controls and the head confusion, read by the Benchmark page.'}</li>
              </ul>
              <Callout variant="note" title={es ? 'Por que se precomputan los embeddings' : 'Why the embeddings are precomputed'}>
                {es
                  ? 'DINOv2/MAE son el techo SOTA pero demasiado pesados para el navegador; se documentan, no se embarcan. Solo se embarcan estadisticos OOD compactos + la cabeza pequeña. El backbone MobileNet (~3.7 MB) si cabe y se carga bajo demanda solo en el carril Real.'
