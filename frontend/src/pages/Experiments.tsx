...
export default function Experiments() {
  ...
  return (
    <article className="page-body prose">
      ...
      <p>{es
        ? 'La contribucion de features-OOD + cabeza real se mide offline (data-pipeline/cllab/science/ood_bench.py) sobre DCID-7 completo, no sobre los pocos parches embarcados. El protocolo evita las fugas que inflarian la exactitud.'
      ...
      <ul className="pf-list">
        <li><b>{es ? 'Split seguro ante fugas' : 'Leakage-safe split'}</b>: {es
          ? 'se usa el split train/test PROPIO de DCID (imagenes fuente distintas), mas deduplicacion por hash perceptual entre lados para eliminar casi-duplicados. Las carpetas de aumentacion RWDA (noise-*) se excluyen por completo.'
      ...
      {ood ? (
        <>
          <p className="mono">{es ? 'Datos' : 'Data'}: synth {ood.data.synthTrain}+{ood.data.synthIdEval} · DCID train {ood.data.dcidTrain} · DCID test {ood.data.dcidTest} · {es ? 'dedupe elimino' : 'dedupe dropped'} {ood.data.dedupeDropped}</p>
          <Callout variant={ood.controls.nullCollapsedToChance ? 'strong' : 'honest'} title={es ? 'Control nulo por permutacion de etiquetas' : 'Label-permutation null control'}>
            {es
          ...
          <p>{es ? 'Metricas OOD: ' : 'OOD metrics: '}<InlineMath tex="\mathrm{AUROC}" />{es ? ' (area bajo ROC), ' : ' (area under ROC), '}<InlineMath tex="\mathrm{AUPR}" />{es ? ' (precision-recall) y ' : ' (precision-recall) and '}<InlineMath tex="\mathrm{FPR}@95\mathrm{TPR}" />{es ? ' (falsos positivos cuando se detecta 95% de OOD). La tabla completa de detectores esta en Benchmark.' : ' (false positives when 95% of OOD is caught). The full detector table is on Benchmark.'}</p>
          <Refs ids={['lee2018', 'sun2022', 'liu2020', 'hendrycks2017', 'li2025dcid']} label={es ? 'Refs' : 'Refs'} />
        </>
      ) : <p className="pf-note">{es ? 'ood-bench.json no cargo (corre ood_bench.py).' : 'ood-bench.json not loaded (run ood_bench.py).'}</p>}
    </article>
  );
}
