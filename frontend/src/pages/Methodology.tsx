      ...
      <Tabs ariaLabel={es ? 'metodología' : 'methodology'} tabs={[
        ...
        },
        {
          ...
          content: (
            <div className="pf-doc-sec">
              <p>{es
                ? 'El clasificador se entrena con textura SINTETICA, pero el core real es de otro dominio. Un patch fuera de distribucion (OOD) debe marcarse, no forzarse a una clase. Partimos de la incertidumbre del softmax y subimos a puntajes en el espacio de features, que la literatura muestra superiores a la reconstruccion.'
                : 'The classifier is trained on SYNTHETIC texture, but real core is another domain. An out-of-distribution (OOD) patch must be flagged, not forced into a class. We start from softmax uncertainty and move up to feature-space scores, which the literature shows beat reconstruction error.'}</p>
              <Equation tex="p_c = \frac{e^{z_c}}{\sum_j e^{z_j}}, \qquad H(x) = -\sum_c p_c \log p_c" caption={es ? 'softmax y entropia predictiva: alta entropia = prediccion insegura' : 'softmax and predictive entropy: high entropy = an unsure prediction'} />
              <p>{es ? 'El baseline de maxima probabilidad softmax (MSP, ' : 'The maximum-softmax-probability baseline (MSP, '}<Cite id="hendrycks2017" paren />{es ? ') marca como novedoso cuando ' : ') flags novelty when '}<InlineMath tex="1-\max_c p_c" />{es ? ' es alto. El puntaje de energia ' : ' is high. The energy score '}<Cite id="liu2020" paren />{es ? ' usa los logits crudos:' : ' uses the raw logits:'}</p>
              <Equation tex="E(x) = -T \cdot \log \sum_c e^{z_c/T}" caption={es ? 'energia: el core en distribucion tiene energia baja; un patch OOD la sube' : 'energy: in-distribution core has low energy; an OOD patch raises it'} />
              <p>{es ? 'El salto clave es medir en el ESPACIO DE FEATURES. La distancia de Mahalanobis ' : 'The key step is to measure in FEATURE SPACE. The Mahalanobis distance '}<Cite id="lee2018" paren />{es ? ' ajusta una Gaussiana por clase con covarianza compartida sobre los embeddings de entrenamiento:' : ' fits a class-conditional Gaussian with shared covariance over the training embeddings:'}</p>
              <Equation tex="M(x) = \min_c\ \big(f(x)-\mu_c\big)^{\top}\,\Sigma^{-1}\,\big(f(x)-\mu_c\big)" caption={es ? 'f(x) es el embedding de 64-d del CNN; mu_c y Sigma se estiman sobre patches sinteticos' : 'f(x) is the CNN 64-d embedding; mu_c and Sigma are estimated over synthetic patches'} />
              <p>{es ? 'El puntaje kNN ' : 'The kNN score '}<Cite id="sun2022" paren />{es ? ' es no parametrico: la distancia al k-esimo vecino mas cercano en un banco de embeddings en distribucion (normalizados L2). Ambos, Mahalanobis y kNN, se comparan contra la reconstruccion incumbente y contra energia/MSP en la pagina Benchmark.'
              ...
              <Callout variant="note" title={es ? 'Adaptacion de dominio (mapa SOTA, no corrido aun)' : 'Domain adaptation (SOTA map, not yet run)'}>
                <p>{es ? 'Para CERRAR la brecha (no solo detectarla), Deep CORAL ' : 'To CLOSE the gap (not only detect it), Deep CORAL '}<Cite id="sun2016coral" paren />{es ? ' alinea los estadisticos de segundo orden de origen y destino, ' : ' aligns the second-order statistics of source and target, '}<InlineMath tex="\ell_{\mathrm{CORAL}} = \tfrac{1}{4d^2}\lVert C_s - C_t \rVert_F^2" />{es ? '; DANN ' : '; DANN '}<Cite id="ganin2016" paren />{es ? ' usa reversion de gradiente para features invariantes al dominio. CoreLog aun no aplica adaptacion; aqui documentamos el mapa. La contribucion real de este build es entrenar una cabeza sobre DCID real (ver Implementacion) y elegir el puntaje OOD que de verdad separa la brecha.'
                  : ' uses gradient reversal for domain-invariant features. CoreLog does not yet apply adaptation; this documents the map. This build\'s real contribution is training a head on real DCID (see Implementation) and selecting the OOD score that actually separates the gap.'}</p>
              </Callout>
        ...
        },
        {
          ...
          content: (
            <div className="pf-doc-sec">
              <p>{es
                ? 'La calidad del puntaje OOD y de la cabeza real depende del ESPACIO DE EMBEDDING. Un backbone congelado y bien entrenado da features donde el core real y el sintetico se separan de forma lineal. Las familias auto-supervisadas y pre-entrenadas son las candidatas.'
              ...
              <p>{es ? 'SimCLR ' : 'SimCLR '}<Cite id="chen2020simclr" paren />{es ? ' aprende por contraste con la perdida NT-Xent sobre pares aumentados:' : ' learns by contrast with the NT-Xent loss over augmented pairs:'}</p>
              <Equation tex="\ell_{i,j} = -\log \frac{\exp(\mathrm{sim}(z_i,z_j)/\tau)}{\sum_{k\neq i}\exp(\mathrm{sim}(z_i,z_k)/\tau)}" caption={es ? 'sim = coseno; tau = temperatura; acerca vistas del mismo patch, aleja las demas' : 'sim = cosine; tau = temperature; pulls together views of the same patch, pushes the rest apart'} />
              <p>{es ? 'DINO ' : 'DINO '}<Cite id="caron2021dino" paren />{es ? ' destila un ViT sin etiquetas (student-teacher con centrado + sharpening); DINOv2 ' : ' self-distills a ViT with no labels (student-teacher with centering + sharpening); DINOv2 '}<Cite id="oquab2023dinov2" paren />{es ? ' escala esto a un backbone de features de proposito general. MAE ' : ' scales this to a general-purpose feature backbone. MAE '}<Cite id="he2022mae" paren />{es ? ' enmascara ~75% de los parches y reconstruye lo faltante, un pre-entrenamiento generativo fuerte.'
                : ' masks ~75% of patches and reconstructs the missing content, a strong generative pretraining.'}</p>
              <Callout variant="honest" title={es ? 'Que se corrio de verdad' : 'What was actually run'}>
                <p>{es ? 'El benchmark de este build usa el embedding de 64-d del propio CNN de litologia (congelado) para el puntaje OOD en vivo, y compara backbones ImageNet congelados (MobileNetV3-Small ' : 'This build\'s benchmark uses the lithology CNN\'s own frozen 64-d embedding for the live OOD score, and compares frozen ImageNet backbones (MobileNetV3-Small '}<Cite id="howard2019mobilenetv3" paren />{es ? ', ResNet18 ' : ', ResNet18 '}<Cite id="he2016resnet" paren />{es ? ') para la cabeza real DCID, todo controlando la resolucion. DINOv2/MAE son la referencia SOTA del techo, demasiado pesados para el navegador; se documentan, no se embarcan. Los numeros medidos estan en Benchmark y Experimentos.'
