      ...
      <Tabs ariaLabel={es ? 'metodología' : 'methodology'} tabs={[
        ...
        },
        {
          ...
          content: (
            <div className="pf-doc-sec">
              <p>{es
                ? 'El clasificador se entrena con textura SINTETICA, pero el core real es de otro dominio. Un patch fuera de distribución (OOD) debe marcarse, no forzarse a una clase. Partimos de la incertidumbre del softmax y subimos a puntajes en el espacio de features, que la literatura muestra superiores a la reconstrucción.'
              ...
              <p>{es ? 'El puntaje kNN ' : 'The kNN score '}<Cite id="sun2022" paren />{es ? ' es no paramétrico: la distancia al k-esimo vecino mas cercano en un banco de embeddings en distribución (normalizados L2). Ambos, Mahalanobis y kNN, se comparan contra la reconstrucción incumbente y contra energía/MSP en la página Benchmark.'
              ...
              <Callout variant="note" title={es ? 'Adaptacion de dominio (mapa SOTA, no corrido aun)' : 'Domain adaptation (SOTA map, not yet run)'}>
                <p>{es ? 'Para CERRAR la brecha (no solo detectarla), Deep CORAL ' : 'To CLOSE the gap (not only detect it), Deep CORAL '}<Cite id="sun2016coral" paren />{es ? ' alinea los estadisticos de segundo orden de origen y destino, ' : ' aligns the second-order statistics of source and target, '}<InlineMath tex="\ell_{\mathrm{CORAL}} = \tfrac{1}{4d^2}\lVert C_s - C_t \rVert_F^2" />{es ? '; DANN ' : '; DANN '}<Cite id="ganin2016" paren />{es ? ' usa reversion de gradiente para features invariantes al dominio. CoreLog aun no aplica adaptacion; aquí documentamos el mapa. La contribucion real de este build es entrenar una cabeza sobre DCID real (ver Implementacion) y elegir el puntaje OOD que de verdad separa la brecha.'
