  ...
  return (
    <article className="page-body prose">
      ...
      <Callout variant="strong" title={es ? 'Toda la CV se ejecuta en vivo en el navegador' : 'All the CV runs live in the browser'}>
        {es
          ? 'El generador de bandejas, la segmentación run-merge y el clasificador se ejecutan en TypeScript en el navegador; el CNN de litología se ejecuta vía onnxruntime-web. Al seleccionar un caso, la segmentación se recalcula al instante. (La carga de una bandeja propia en la app aún no está implementada.)'
          : 'The tray generator, the run-merge segmentation and the classifier run in TypeScript in the browser; the lithology CNN runs via onnxruntime-web. Pick a case and the segmentation re-computes instantly. (In-app upload of a user-supplied tray is not implemented yet.)'}
      </Callout>
