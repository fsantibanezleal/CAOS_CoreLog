    ...
    <article className="page-body prose">
      ...
      {learned ? (
        ...
        <Callout variant="honest" title={es ? 'Métricas aprendidas no disponibles' : 'Learned metrics unavailable'}>
          {es
            ? 'cl-learned.json no cargó en esta sesión, los modelos entrenados (torch · ONNX) vienen versionados con el build; ejecutar `python -m pipeline.pipeline all --retrain` para regenerarlos. El ground-truth del generador es siempre la verdad de terreno.'
            : 'cl-learned.json did not load in this session, the trained models (torch · ONNX) ship committed with the build; run `python -m pipeline.pipeline all --retrain` to regenerate them. The generator ground truth is always the authority.'}
        </Callout>
      )}
