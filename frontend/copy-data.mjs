...
// loads them. Canonical copies live in ../data/derived, public/ is a build-time overlay (git-ignored). CoreLog's live
// lane is the TypeScript CV engine (frontend/src/cv/) + onnxruntime-web; there is no Pyodide lane to inline.
...
if (!existsSync(derived)) {
  console.warn('[copy-data] no data/derived, run `npm run bake` (or `python -m cllab.pipeline all`) first');
} else {
