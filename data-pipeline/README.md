...
Python re-port). `pipeline` orchestrates the bake, applies the contracts, and reshapes the committed outputs into replay
traces.
...
- `pipeline/io/`, `contract.py` (**CONTRACT 1**: tray descriptor + dropped image) · `formats.py` · `schema.py`
- `pipeline/core/`, `rng.py` · `trace.py` · `manifest.py` (**CONTRACT 2**) · `gate.py` (live/precompute gate)
- `pipeline/model/`, `learned.py` (the 2 learned models' patch contracts, the source of truth the SPA reproduces)
- `pipeline/stages/`, `preprocess → feature_extraction → train → infer → evaluate → export` (thin over the science)
- `pipeline/science/`, `bake_cases.mjs` · `gen_train.mjs` (Node + tsx, the SAME TS engine) · `train_litho.py` (torch → ONNX)

## The default lane is light

`python data-pipeline/run.py all` reshapes the committed `data/derived/case-results.json` + `cl-learned.json` into
